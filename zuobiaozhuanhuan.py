...
# 读取原始CSV文件
with open(r"E:\上课资料\kongjainfenxi_gdb\dengshiquangnaichadian\raw_residential.csv", 'r',encoding='utf-8') as file:
...
for row in data:
    ...
    #converted_data.append([row[0], row[1], row[2],row[4],row[3],row[5],row[6],row[7],gcj_lng,gcj_lat])
    converted_data.append([row[0], row[1], row[3],row[2],row[4],row[5],wgs84_lng,wgs84_lat])

# 保存转换后的数据到新的CSV文件
with open(r"E:\上课资料\kongjainfenxi_gdb\dengshiquangnaichadian\repo_residential.csv", 'w', newline='',encoding='utf-8') as file:
    writer = csv.writer(file)
    #writer.writerow(['name','class','location','bdlng','bdlat','uid','wgs84_lng','wgs84_lat','gcj_lng','gcj_lat'])  # 写入表头
    writer.writerow(['name','location','bdlng','bdlat','uid','class','wgs84_lng','wgs84_lat'])
